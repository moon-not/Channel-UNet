...
def getArgs():
    ...
    parse.add_argument('--dataset', default='seg',  # dsb2018_256
                       help='dataset name:liver/esophagus/dsb2018Cell/corneal/driveEye/isbiCell/kaggleLung/seg')
...
def getModel(args):
    ...
    if args.arch =='Attention_UNet':#比较好
...
def getDataset(args):
    ...
    if args.dataset == 'kaggleLung':
        ...
        test_dataloaders = DataLoader(test_dataset, batch_size=1)
    if args.dataset == 'seg':
        train_dataset = segDataset(r'train', transform=x_transforms, target_transform=y_transforms)
        train_dataloaders = DataLoader(train_dataset, batch_size=args.batch_size)
        val_dataset = segDataset(r"val", transform=x_transforms, target_transform=y_transforms)
        val_dataloaders = DataLoader(val_dataset, batch_size=1)
        test_dataset = segDataset(r"test", transform=x_transforms, target_transform=y_transforms)
        test_dataloaders = DataLoader(test_dataset, batch_size=1)
...
def test(val_dataloaders,save_predict=False):
    ...
    with torch.no_grad():
        ...
        for pic,_,pic_path,mask_path in val_dataloaders:
            ...
            matplotlib.image.imsave('%r.png' % (n), predict,cmap='Greys_r')
            n=n+1
            fig = plt.figure()
